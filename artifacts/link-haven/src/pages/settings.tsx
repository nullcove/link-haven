...
._s-acc-body { overflow: hidden; transition: max-height .32s cubic-bezier(.4,0,.2,1), opacity .28s ease; }
`;
...
function UserAvatar({ name, size = 64 }: { name: string; size?: number }) {
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <div className="relative rounded-full flex items-center justify-center font-black text-white overflow-hidden"
        style={{ width: size, height: size, fontSize: Math.round(size * 0.35), letterSpacing: "-0.02em",
        ...
        <span className="absolute inset-0 rounded-full" style={{ background: "linear-gradient(145deg, rgba(255,255,255,.28) 0%, transparent 50%)" }} />
        <span className="absolute bottom-0 inset-x-0 rounded-full" style={{ height: "45%", background: "linear-gradient(to top, rgba(0,0,0,.25), transparent)" }} />
      ...
      <span className="absolute border-[3px] border-[#08080e] rounded-full bg-emerald-400"
        style={{ width: dot, height: dot, bottom: 0, right: 0, boxShadow: "0 0 8px #34d399, 0 0 16px #34d39960" }} />
    </div>
  );
}

/* ─── Icon3D ──────────────────────────────────────────────────── */
function Icon3D({ icon: Ic, color, size = 36, anim = "none", delay = 0 }: {
  ...
  return (
    <span className="_s-icon _s-popIn relative flex items-center justify-center shrink-0"
      style={{ width: size, height: size, borderRadius: r, animationDelay: `${delay}ms`,
        ...
        boxShadow: `0 4px 16px ${color}35, 0 2px 6px ${color}28, inset 0 2px 0 rgba(255,255,255,.18), inset 0 -2px 0 rgba(0,0,0,.22)` }}>
      ...
      <Ic className={anim !== "none" ? `_s-${anim}` : ""} style={{ width: size * 0.5, height: size * 0.5, color: "#fff", filter: `drop-shadow(0 2px 6px ${color}70)`, position: "relative", zIndex: 1, flexShrink: 0 }} strokeWidth={1.8} />
    </span>
  );
}

/* ─── BarChartIcon3D ──────────────────────────────────────────── */
function BarChartIcon3D({ color, size = 36 }: { color: string; size?: number }) {
  const r = Math.round(size * 0.3);
  return (
    <span className="_s-icon _s-popIn relative flex items-center justify-center shrink-0"
      style={{ width: size, height: size, borderRadius: r, background: `radial-gradient(circle at 32% 28%, ${color}65, transparent 55%), linear-gradient(145deg, ${color}45, ${color}1c)`, border: `1.5px solid ${color}48`, boxShadow: `0 4px 16px ${color}35, 0 2px 6px ${color}28, inset 0 2px 0 rgba(255,255,255,.18), inset 0 -2px 0 rgba(0,0,0,.22)` }}>
      <span className="absolute inset-0 pointer-events-none overflow-hidden" style={{ borderRadius: r, background: "linear-gradient(138deg, rgba(255,255,255,.28) 0%, transparent 46%)" }} />
      <span className="relative z-10 flex items-end gap-[2px]" style={{ height: size * 0.46, width: size * 0.46 }}>
        {[{ a: "_s-barA", d: "0ms" }, { a: "_s-barB", d: "180ms" }, { a: "_s-barC", d: "90ms" }].map((bar, i) => (
          <span key={i} className="flex-1 rounded-sm" style={{ height: "40%", background: `linear-gradient(to top, ${color}, rgba(255,255,255,.8))`, animation: `${bar.a} 1.6s ease-in-out infinite`, animationDelay: bar.d }} />
        ))}
      </span>
    </span>
  );
...
/* ─── AccordionSection ────────────────────────────────────────── */
function AccordionSection({ iconEl, title, color, badge, defaultOpen = false, children }: {
  ...
  const [maxH, setMaxH] = useState(defaultOpen ? "2000px" : "0px");

  useEffect(() => {
    if (open) {
      const h = bodyRef.current?.scrollHeight ?? 2000;
      setMaxH(`${h + 40}px`);
    } else {
      setMaxH("0px");
    }
  ...
  return (
    <div className="rounded-2xl overflow-hidden _s-popIn"
      style={{ background: "linear-gradient(145deg, rgba(255,255,255,.035) 0%, rgba(255,255,255,.012) 100%)", border: "1px solid rgba(255,255,255,.08)", boxShadow: "0 4px 24px rgba(0,0,0,.38), inset 0 1px 0 rgba(255,255,255,.05)", backdropFilter: "blur(14px)" }}>

      {/* ── Header / trigger ── */}
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center gap-3 px-5 py-3.5 text-left transition-all hover:brightness-110 active:brightness-95"
        style={{ background: open ? `linear-gradient(90deg, ${color}14, transparent 70%)` : "transparent" }}>
        {/* icon */}
        {iconEl}

        {/* label */}
        <div className="flex-1 min-w-0">
          <span className="text-[13.5px] font-black text-white leading-none">{title}</span>
          {badge && !open && (
        ...
        <ChevronDown
          ...
          style={{ color: `${color}90`, transform: open ? "rotate(180deg)" : "rotate(0deg)" }}
        />
      </button>
...
function StatTile({ icon: Ic, label, value, color }: { icon: React.ElementType; label: string; value: number; color: string }) {
  ...
  return (
    <div className="relative rounded-xl overflow-hidden border border-white/[0.06] p-3.5 flex flex-col gap-2 cursor-default"
      style={{ background: "linear-gradient(145deg, rgba(255,255,255,.025), rgba(255,255,255,.01))", transition: "transform .18s ease, box-shadow .18s ease", transform: hov ? "translateY(-2px) scale(1.02)" : "", boxShadow: hov ? `0 8px 24px ${color}22` : "", backdropFilter: "blur(8px)" }}
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}>
      <div className="absolute top-0 inset-x-0 h-0.5" style={{ background: `linear-gradient(90deg, ${color}70, transparent)` }} />
      <span className="size-7 flex items-center justify-center rounded-[8px] relative shrink-0"
        ...
        <Ic style={{ width: 13, height: 13, color: "#fff", filter: `drop-shadow(0 1px 3px ${color}70)`, position: "relative", zIndex: 1 }} strokeWidth={2} />
      </span>
      <span className="text-[26px] font-black leading-none" style={{ color, textShadow: `0 0 20px ${color}50` }}>{value.toLocaleString()}</span>
      <span className="text-[10px] text-white/35 font-semibold leading-none">{label}</span>
    </div>
  );
}
...
function BgPicker() {
  ...
  return (
    <div className="space-y-4">
      {/* Active preview */}
      {bgItem ? (
        <div className="relative rounded-xl overflow-hidden border border-white/[0.08]" style={{ height: 80 }}>
          <img src={bgItem.path} alt={bgItem.name} className="absolute inset-0 w-full h-full object-cover" draggable={false} />
          <div className="absolute inset-0" style={{ background: "linear-gradient(to right, rgba(4,4,11,.85) 0%, rgba(4,4,11,.35) 65%, transparent 100%)" }} />
          <div className="absolute inset-0 flex items-center justify-between px-4">
            <div>
              <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-white/40 mb-0.5">Active Wallpaper</p>
              <p className="text-[14px] font-black text-white leading-none">{bgItem.name}</p>
              <p className="text-[10px] mt-0.5 font-semibold" style={{ color: BG_CATEGORY_META[bgItem.category].accent }}>
                {BG_CATEGORY_META[bgItem.category].glyph} {bgItem.category}
              </p>
            </div>
            <button onClick={() => setBg(null)}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10.5px] font-bold border transition-all hover:scale-105"
              style={{ background: "rgba(0,0,0,.55)", borderColor: "rgba(255,255,255,.12)", color: "rgba(255,255,255,.45)", backdropFilter: "blur(8px)" }}>
              <X className="size-2.5" /> Remove
            </button>
          </div>
        </div>
      ) : (
        <div className="flex items-center gap-3 px-3.5 py-2.5 rounded-xl border border-white/[0.05]"
          style={{ background: "rgba(255,255,255,.02)" }}>
          <X className="size-3.5 text-white/18 shrink-0" />
          <p className="text-[11.5px] text-white/25">No wallpaper — using solid dark background</p>
        </div>
      )}
        ...
        return (
          <div key={cat}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[12px] font-black" style={{ color: meta.accent }}>{meta.glyph}</span>
              <span className="text-[10.5px] font-black text-white/60 uppercase tracking-[0.15em]">{cat}</span>
              <span className="text-[9.5px] text-white/18">{items.length}</span>
              <div className="flex-1 h-px" style={{ background: `linear-gradient(to right, ${meta.accent}22, transparent)` }} />
            </div>
              ...
              {items.map(bg => {
                ...
                return (
                  <button key={bg.id} onClick={() => setBg(sel ? null : bg.id)}
                    className="_s-bg-card relative rounded-lg overflow-hidden cursor-pointer"
                    style={{ paddingBottom: "56.25%", border: `2px solid ${sel ? meta.accent : "rgba(255,255,255,.07)"}`, boxShadow: sel ? `0 0 0 1px ${meta.accent}45, 0 4px 16px ${meta.accent}30` : "" }}>
                    <img src={bg.path} alt={bg.name} className="absolute inset-0 w-full h-full object-cover" draggable={false} loading="lazy" />
                    <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(0,0,0,.72) 0%, rgba(0,0,0,.06) 55%)" }} />
                    {sel && (
                      <>
                        <div className="absolute inset-0" style={{ background: `${meta.accent}16` }} />
                        <span className="_s-bg-sel absolute top-1.5 right-1.5 size-4 rounded-full flex items-center justify-center"
                          style={{ background: meta.accent, boxShadow: `0 0 8px ${meta.accent}80` }}>
...
function ViewToggle({ value, selected, onSelect, icon: Ic, label }: { value: string; selected: boolean; onSelect: (v: string) => void; icon: React.ElementType; label: string }) {
  return (
    <button onClick={() => onSelect(value)}
      className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[12px] font-semibold border transition-all hover:scale-[1.02]"
      style={{ background: selected ? "linear-gradient(135deg, rgba(99,102,241,.2), rgba(139,92,246,.12))" : "rgba(255,255,255,.03)", borderColor: selected ? "rgba(99,102,241,.4)" : "rgba(255,255,255,.08)", color: selected ? "#a5b4fc" : "rgba(255,255,255,.4)", boxShadow: selected ? "0 4px 12px rgba(99,102,241,.18), inset 0 1px 0 rgba(255,255,255,.06)" : "" }}>
...
export default function SettingsPage() {
  ...
  return (
    <AppLayout>
      ...
      {/* ── Sticky header ── */}
      <header className="h-13 shrink-0 border-b border-white/[0.06] flex items-center gap-3 px-5 sticky top-0 z-10"
        style={{ background: "rgba(7,7,14,.88)", backdropFilter: "blur(20px)" }}>
        <Icon3D icon={Settings} color="#94a3b8" size={28} anim="spin" />
        <div>
          <h1 className="font-black text-[14px] text-white leading-none">Settings</h1>
          <p className="text-[9.5px] text-white/22 mt-px">Account &amp; preferences</p>
        </div>
      </header>

      <div className="flex-1 overflow-y-auto py-4 px-4" style={{ background: "transparent" }}>
        <div className="max-w-2xl mx-auto space-y-2.5">

          {/* ── 1. Profile ── */}
          <AccordionSection defaultOpen title="Profile" color="#6366f1"
            badge={user ? <span className="text-[10.5px] text-white/30 truncate">{user.name}</span> : undefined}
            iconEl={<Icon3D icon={UserCircle2} color="#6366f1" size={32} anim="float" delay={80} />}>
            <div className="flex items-start gap-4 mb-4 mt-2">
              {user && <UserAvatar name={user.name} size={64} />}
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11.5px] text-white/32 mt-1.5 flex items-center gap-1.5">
                  <Mail className="size-3 shrink-0" style={{ color: "rgba(255,255,255,.22)" }} />
                ...
                <div className="flex items-center gap-2 mt-2">
                  <span className="inline-flex items-center gap-1.5 text-[9.5px] font-black px-2.5 py-1 rounded-full"
                    style={{ background: user?.isGuest ? "rgba(100,116,139,.18)" : "linear-gradient(135deg,rgba(99,102,241,.25),rgba(139,92,246,.15))", color: user?.isGuest ? "#94a3b8" : "#c4b5fd", border: `1px solid ${user?.isGuest ? "rgba(100,116,139,.3)" : "rgba(99,102,241,.4)"}` }}>
                    {user?.isGuest ? "👤 Guest" : <><Sparkles className="size-2.5" />Pro Member</>}
                  </span>
                  {user && !user.isGuest && (
                    <span className="text-[10px] text-white/20 flex items-center gap-1">
                      <CalendarDays className="size-3" />Since {format(new Date(user.createdAt), "MMM yyyy")}
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="grid gap-2.5">
              {[
                ...
                <div key={label}>
                  <label className="text-[9.5px] font-bold text-white/28 block mb-1 uppercase tracking-wider">{label}</label>
          ...
          <AccordionSection defaultOpen title="Background Wallpaper" color="#06b6d4"
            badge={
              bgItem
                ? <span className="text-[10.5px]" style={{ color: BG_CATEGORY_META[bgItem.category].accent }}>
                    {BG_CATEGORY_META[bgItem.category].glyph} {bgItem.name}
                  </span>
                : <span className="text-[10.5px] text-white/25">No wallpaper set</span>
            }
            iconEl={<Icon3D icon={ImageIcon} color="#06b6d4" size={32} anim="breathe" delay={100} />}>
            <div className="mt-2">
              <BgPicker />
            </div>
          </AccordionSection>
          ...
          <AccordionSection title="AI Models" color="#a855f7"
            badge={<span className="text-[10.5px] text-white/25">10 providers available</span>}
            iconEl={<Icon3D icon={Bot} color="#a855f7" size={32} anim="breathe" delay={150} />}>
              ...
              {Object.entries(LOGOS).map(([name]) => {
                ...
                return (
                  <div key={name} className="flex flex-col items-center gap-1.5 p-2 rounded-xl border cursor-default transition-all hover:scale-105"
                    ...
                    <span className="text-[8.5px] font-semibold text-center leading-tight" style={{ color: "rgba(255,255,255,.35)" }}>{name}</span>
                  </div>
                );
              })}
            </div>
            <Link href="/ai-settings"
              ...
              style={{ background: "linear-gradient(135deg, rgba(168,85,247,.12), rgba(139,92,246,.07))", borderColor: "rgba(168,85,247,.25)" }}
              ...
              <div className="flex items-center gap-3">
                <span className="size-7 rounded-lg flex items-center justify-center relative"
                  style={{ background: "radial-gradient(circle at 32% 28%, rgba(168,85,247,.5), transparent 55%), linear-gradient(145deg, rgba(168,85,247,.35), rgba(168,85,247,.12))", border: "1px solid rgba(168,85,247,.4)" }}>
                  <Key className="size-3.5 text-purple-300 relative z-10" strokeWidth={2} />
                </span>
                <div>
                  <p className="text-[12.5px] font-bold text-purple-300 group-hover:text-purple-200 leading-none">Manage API keys &amp; models</p>
                  <p className="text-[10px] text-white/25 mt-0.5">Configure providers, test connections</p>
                </div>
              </div>
              <ChevronDown className="size-4 text-purple-400/40 group-hover:text-purple-400 -rotate-90 transition-all" />
            </Link>
          </AccordionSection>

          {/* ── 4. Statistics ── */}
          <AccordionSection title="Usage Statistics" color="#06b6d4"
            badge={stats ? <span className="text-[10.5px] text-white/25">{stats.totalBookmarks} bookmarks · {stats.totalCollections} collections</span> : undefined}
            iconEl={<BarChartIcon3D color="#06b6d4" size={32} />}>
          ...
          <AccordionSection title="Preferences" color="#ec4899"
            badge={<span className="text-[10.5px] text-white/25">Default view: {defaultView}</span>}
            iconEl={<Icon3D icon={Palette} color="#ec4899" size={32} anim="breathe" delay={220} />}>
            <div className="mt-2">
              <label className="text-[9.5px] font-bold text-white/28 block mb-2 uppercase tracking-wider">Default Bookmark View</label>
          ...
          <AccordionSection title="Keyboard Shortcuts" color="#10b981"
            badge={<span className="text-[10.5px] text-white/25">7 shortcuts</span>}
            iconEl={<Icon3D icon={Keyboard} color="#10b981" size={32} anim="none" delay={260} />}>
            <div className="mt-2">
              {[
                ...
                <div key={key} className="flex items-center justify-between py-2 border-b border-white/[0.04] last:border-0">
                  ...
                  <kbd className="inline-flex items-center gap-1 text-[10px] px-2.5 py-1 rounded-lg font-mono"
          ...
          <AccordionSection title="Security" color="#f59e0b"
            badge={<span className="text-[10.5px] text-white/25">2FA coming soon</span>}
            iconEl={<Icon3D icon={Shield} color="#f59e0b" size={32} anim="float" delay={290} />}>
            <div className="flex items-center justify-between p-3.5 rounded-xl border border-white/[0.06] mt-2"
              style={{ background: "rgba(255,255,255,.02)" }}>
              <div className="flex items-center gap-3">
                <span className="size-8 rounded-xl flex items-center justify-center relative"
                  style={{ background: "radial-gradient(circle at 32% 28%, rgba(245,158,11,.45), transparent 58%), linear-gradient(145deg, rgba(245,158,11,.3), rgba(245,158,11,.1))", border: "1px solid rgba(245,158,11,.3)" }}>
                  <Zap className="size-3.5 text-amber-300 relative z-10" strokeWidth={2} />
                </span>
                <div>
                  <p className="text-[12.5px] font-bold text-white/72">Two-factor authentication</p>
                  <p className="text-[10px] text-white/28 mt-0.5">Add extra security to your account</p>
                </div>
              </div>
