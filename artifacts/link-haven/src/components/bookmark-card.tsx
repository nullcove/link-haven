...
const TYPE_COLORS: Record<string, string> = {
  link: "#6366f1", article: "#8b5cf6", image: "#10b981", video: "#ef4444", document: "#f59e0b",
};
const TYPE_ICONS = { link: Link2, article: FileText, image: ImageIcon, video: Video, document: File };
...
function Domain({ bookmark }: { bookmark: Bookmark }) {
  const domain = (() => {
...
export function BookmarkCard({ bookmark, onClick, onToggleFavorite, onToggleArchive, onDelete, viewMode }: BookmarkCardProps) {
  ...
  if (viewMode === "list") {
    return (
      <motion.div
        ...
        className="group flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/[0.04] border border-transparent hover:border-white/[0.07] transition-all cursor-pointer"
        onClick={onClick}
      >
        {/* Favicon */}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-[11px] text-white/30">
            ...
            {bookmark.collectionName && <><span className="text-white/15">·</span><span className="text-indigo-400/60 truncate">{bookmark.collectionName}</span></>}
          </div>
        </div>
        ...
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          ...
          <ActionBtn onClick={e => { e.stopPropagation(); onToggleFavorite(); }} title="Favourite" active={bookmark.isFavorite} activeColor="text-amber-400">
            <Star className={`size-3.5 ${bookmark.isFavorite ? "fill-amber-400" : ""}`} />
          </ActionBtn>
          <MoreMenu bookmark={bookmark} onToggleArchive={onToggleArchive} onDelete={onDelete} />
        </div>
      </motion.div>
    );
  }
  ...
  return (
    <motion.div layout initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.15 }}>
      <div
        className="group h-full rounded-xl border border-white/[0.07] bg-[#0f0f1c] hover:border-indigo-500/20 hover:bg-[#11111f] transition-all cursor-pointer flex flex-col overflow-hidden"
        onClick={onClick}
      >
        {/* Cover / Placeholder */}
...
function MoreMenu({ bookmark, onToggleArchive, onDelete }: {
  bookmark: Bookmark;
  onToggleArchive?: () => void;
  onDelete: () => void;
}) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-40 bg-[#131320] border border-white/10 shadow-2xl rounded-lg p-1">
        <DropdownMenuItem
